...
def sarsa(n_timesteps, learning_rate, gamma, policy='egreedy', epsilon=None, temp=None, plot=True):
    ...
    for t in range(n_timesteps):
        ...
        action_next = pi.select_action(state_next, policy=policy, epsilon=epsilon, temp=temp)
        pi.update(state, action, reward, state_next, action_next, done)
        rewards.append(reward)

        state = env.reset() if done else state_next

        if plot and t % 1000 == 0:
           env.render(Q_sa=pi.Q_sa, plot_optimal_policy=True,
                      step_pause=0.1)  # Plot the Q-value estimates during SARSA execution

    if plot:
       env.render(Q_sa=pi.Q_sa,plot_optimal_policy=True,step_pause=30) # Plot the Q-value estimates during SARSA execution
    # env.render(Q_sa=pi.Q_sa, plot_optimal_policy=True, step_pause=100)  # Plot the Q-value estimates during SARSA execution
